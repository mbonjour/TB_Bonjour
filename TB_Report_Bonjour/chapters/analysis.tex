...
Pour mes tests j’ai fait en sorte de trouver l’utilisation la plus simple possible pour voir si un utilisateur lambda pouvait arriver à mettre en place ce genre de sécurité. Il s’est avéré que cela était assez simple au départ, mais dès lors que l'on veut envoyer un mail chiffré à un correspondant cela se complique un peu. J’ai juste eu à installer un Add-On sur mon logiciel de messagerie (Thunderbird dans mon cas) qui s’appelle Enigmail. Ensuite Enigmail a générer mes clés PGP (de manière totalement automatisée). Puis j’ai écrit un mail, en appuyant sur un petit cadenas mon mail partait chiffré et signé (uniquement si on a la clé du correspondant). Bien, cependant c’est très opaque et on ne sait pas ce qu'Enigmail fait réellement derrière les décors. L’utilisateur doit encore choisir s’il veut chiffrer ses mails ou non par contre il faut que le destinataire utilise PGP et que l’on ait sa clé publique. De plus, Enigmail utilise Autocrypt qui envoie notre clé publique directement dans le mail, ce qui n'est pas forcément pris en compte pour tout les destinataires et pourrait sujet à des modifications. Pour les clés Enigmail les envoie sur des serveurs de clés par défaut et va d'ailleurs regarder ces serveurs si la clé publique d'un destinataire n'est pas stockée en premier lieu. Ces serveurs sont les suivants : keys.opengpg.org (vks), hkps.pool.sks-keyservers.net (hkps), pgp.mit.edu (hkps). vks (Verifying Keyservers) et hkps (HTTP keyserver protocol over TLS) sont des interfaces avec des serveurs de clés afin d'enregistrer des nouvelles clés ou trouver une clé sur le serveur par différents moyens (email, key-id, fingerprint).
\subsection{S/MIME}
\label{protocols:SMIME}
\paragraph*{Fonctionnement.}
S/MIME se base sur un système de PKI (Public Key Infrastructure), c'est à dire en utilisant des CAs (Certificate authorities) pour signer des certificats mettant en relation un email à une clé publique. De là tout le système fonctionne comme SSL/TLS, cela veut dire :
\paragraph*{Propriétés cryptographiques.}
...
J'ai testé  un plugin firefox qui permet d'avoir des certificats pour GMail et envoyer des mails signés et chiffrés à l'aide de S/MIME. Cependant les certificats ne sont pas vérifier correctement comme on peut le voir à la réception dans GMail à la figure \ref{fig:SMIME_MeSinceProblem}.
\begin{figure}[h!]
